...
<div class="files view large-9 medium-8 columns content">
    ...
    <div class="related">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($file->childrens as $childrens): ?>
            <tr>
                <td><?= h($childrens->id) ?></td>
                <td><?= h($childrens->user_id) ?></td>
                <td><?= h($childrens->title) ?></td>
                <td><?= h($childrens->slug) ?></td>
                <td><?= h($childrens->body) ?></td>
                <td><?= h($childrens->published) ?></td>
                <td><?= h($childrens->created) ?></td>
                <td><?= h($childrens->modified) ?></td>
                <td class="actions">
                    ...
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Childrens', 'action' => 'delete', $childrens->id], ['confirm' => __('Are you sure you want to delete # {0}?', $articles->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
